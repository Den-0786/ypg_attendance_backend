...
export default function YearEndChart({ attendanceData, darkMode }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className='p-2 rounded-xl shadow-2xl border backdrop-blur-sm bg-gray-900 border-gray-700 text-white'>
          <p className="font-bold text-lg mb-3 text-center">{label}</p>
          <div className="space-y-2 text-sm">
            <div className="grid grid-cols-2 gap-4">
              <div>
  ...
  const exportData = () => {
    const csvContent = [
      ['Congregation', 'Total Meetings', 'Attendance Rate (%)', ...months],
      ...chartData.map(data => [
        data.congregation,
        data.totalMeetings,
        data.attendanceRate,
