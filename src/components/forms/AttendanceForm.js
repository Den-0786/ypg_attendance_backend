...
export default function AttendanceForm({ meetingInfo }) {
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
      const payload = attendees.map(
        ({ meetingDate, meetingTitle, ...rest }) => ({
          ...rest,
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6  max-w-xl mx-auto border border-yellow-200">
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 mb-6">
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          <p className="text-gray-700">
            Please select one of the following and proceed to fill the
            attendance forms
          </p>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-6 text-green-700 bg-green-50 px-4 py-2 rounded-full shadow-sm">
              ...
              <label
                htmlFor="attendance-type-district"
                className="flex items-center gap-2 cursor-pointer"
              >
                <input
                  type="radio"
      ...
      <div className="w-3/3 sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto space-y-4 bg-white p-6 rounded-2xl shadow-xl border border-gray-200">
        <h2 className="text-2xl text-green-700 font-bold text-center mb-2">
          Attendance Form
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-gray-900">
          ...
          <label
            htmlFor="attendance-congregation"
      ...
      {/* ✅ Confirm Modal */}
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-start sm:items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white p-6 rounded-lg w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto my-auto">
            ...
            {pendingEntry && (
              <>
                <div className="text-sm text-gray-800 space-y-1 mb-4 p-3 bg-blue-50 rounded">
                  ...
                  <p>
                    <strong>Name:</strong> {pendingEntry.name}
                  </p>
                  <p>
                    <strong>Congregation:</strong> {pendingEntry.congregation}
                  </p>
                  <p>
                    <strong>Representing:</strong> {pendingEntry.type}
                  </p>
                  <p>
                    <strong>Position:</strong> {pendingEntry.position}
                  </p>
                  <p className="text-xs text-gray-500">
                    <strong>Meeting Date:</strong> {pendingEntry.meetingDate}
                  </p>
                  <p className="text-xs text-gray-500">
                    <strong>Reported @:</strong> {pendingEntry.timestamp}
                  </p>
                </div>
              ...
              <>
                <div className="mb-4">
                  <h4 className="text-md font-semibold text-gray-700 mb-2">
                    Current Entries ({attendees.length})
                  </h4>
                </div>
                <ul className="space-y-3 max-h-72 overflow-y-auto">
                  {attendees.map((entry, index) => (
                    <li
                      key={entry.id}
                      className="p-3 bg-gray-50 rounded shadow flex justify-between items-start relative"
                    >
                      <div className="text-sm text-gray-800 space-y-1">
                        ...
                        <p>
                          <strong>Representing:</strong> {entry.type}
                        </p>
                        <p>
                          <strong>Position:</strong> {entry.position}
                        </p>
                        <p className="text-xs text-gray-500">
                          <strong>Meeting Date:</strong> {entry.meetingDate}
                        </p>
                        <p className="text-xs text-gray-500">
                          <strong>Reported @:</strong> {entry.timestamp}
                        </p>
                      </div>
                      <div className="relative">
                        ...
                        {menuOpenIndex === index && (
                          <div
                            className="absolute right-0 top-6 bg-white shadow-lg rounded-md w-40 z-[9999] border border-gray-200"
